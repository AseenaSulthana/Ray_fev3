...
//
// After a file lands in Google Drive we call the RAG backend's /ingest
// endpoint so it is chunked + embedded immediately — without waiting for
// the slow drive-watcher polling cycle.
// ---------------------------------------------------------------------------

const RAG_SERVICE_URL = process.env.RAG_SERVICE_URL ?? 'http://127.0.0.1:8000'

async function triggerRagIngest(params: {
  fileId: string
  fileName: string
  mimeType: string
  fileUrl: string
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn('[documents/POST] RAG ingest returned non-OK:', res.status, text)
    } else {
      const data = await res.json().catch(() => null)
      console.log(
        '[documents/POST] RAG ingest triggered:',
        data?.chunks_stored ?? '?',
        'chunks stored for',
        params.fileName,
      )
    ...
    // Ingest failure must NOT break the upload response — log and continue.
    console.warn('[documents/POST] RAG ingest trigger failed (non-fatal):', err)
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ── Trigger RAG ingestion (fire-and-forget) ──────────────────────────
    if (uploaded.driver === 'drive' && uploaded.fileId) {
