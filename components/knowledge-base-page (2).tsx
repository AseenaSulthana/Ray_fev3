...
export function KnowledgeBasePage({ role = 'employee', currentUser }: KnowledgeBasePageProps) {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
    >
      {/* Full-page drag overlay — role-themed */}
      {isDragging && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: role === 'admin' ? 'rgba(123,91,255,0.07)' : 'rgba(0, 184, 212, 0.08)', backdropFilter: 'blur(2px)', pointerEvents: 'none' }}>
          <div style={{ background: rayColors.white, borderRadius: 16, border: `2px dashed ${role === 'admin' ? rayColors.purple500 : rayColors.cyan500}`, padding: '40px 60px', textAlign: 'center', boxShadow: role === 'admin' ? '0 8px 32px rgba(123,91,255,0.15)' : '0 8px 32px rgba(0, 184, 212, 0.15)' }}>
            <CloudUpload size={40} style={{ color: role === 'admin' ? rayColors.purple500 : rayColors.cyan500, margin: '0 auto 12px', display: 'block' }} />
      ...
      <div style={innerWrap}>
        ...
        {/* ── DRAG AND DROP UPLOAD ZONE (role-themed) ── */}
        {(() => {
          // Role-specific accent colours for the drag zone
          const zoneAccent = role === 'admin'
            ? { active: rayColors.purple500, activeBg: rayColors.purple100, activeBorder: rayColors.purple400, activeFg: rayColors.purple700, idleBg: rayColors.purple50, icon: rayColors.inkMuted }
            : role === 'executive'
            ? { active: '#0891b2', activeBg: '#e0f7fa', activeBorder: '#06b6d4', activeFg: '#0e7490', idleBg: '#f0fdff', icon: rayColors.inkMuted }
            : { active: rayColors.cyan500, activeBg: rayColors.cyan50, activeBorder: rayColors.cyan400, activeFg: rayColors.cyan700, idleBg: rayColors.purple50, icon: rayColors.inkMuted }
          ...
          return (
            <label
              style={{
                ...
                padding: '28px 20px',
                borderRadius: 12,
                border: `2px dashed ${isDragging ? zoneAccent.activeBorder : tint.cardBorder}`,
                background: isDragging ? zoneAccent.activeBg : zoneAccent.idleBg,
                cursor: 'pointer',
                transition: 'all 0.2s ease',
              }}
            >
              <CloudUpload size={28} style={{ color: isDragging ? zoneAccent.active : zoneAccent.icon }} />
              <div style={{ textAlign: 'center' }}>
                <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: isDragging ? zoneAccent.activeFg : rayColors.ink }}>
                  Drag files here, or{' '}
                  <span style={{ color: zoneAccent.active, textDecoration: 'underline' }}>browse to upload</span>
                </p>
